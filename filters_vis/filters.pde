...
public class CosineBell extends AFilter {
    ...
    private double cosinebell(double x) {
        ...
        if (xx>xm) return 0;
        return (1.0+Math.cos(Math.PI*xx/xm))/2.0;
    }

    public double evaluate(double x, double y) {
        return cosinebell(Math.sqrt(x*x+y*y));
    }
}
...
public class Mitchell extends AFilter {
    ...
    public Mitchell(double radius, double factor, double B, double C) {
        super(radius);
        this.factor = 1.0/factor;
        this.B = B;
        this.C = C;
    ...
    public double evaluate(double x, double y) {
        return Mitchell1d(x*factor) * Mitchell1d(y*factor);
    }
}
